...
{
	$txtid = NextID('iAreaID', 'area');
	$txtname = db_input2($_POST['txtname']);
	$cmbprop = $_POST['cmbprop'];
	$txtrank = $txtid;
	$rdstatus =  db_input($_POST['rdstatus']);	

	$q = "insert into area (iAreaID, vName, iPropertyID, iRank, cStatus) values ($txtid, '$txtname', '$cmbprop', '$txtrank', '$rdstatus')";
	$r = sql_query($q, "COMPANY.70");
	
	$_SESSION[PROJ_SESSION_ID]->success_info = "Area Details Successfully Inserted";
}
else if($mode=='E')
{
	$dataArr = GetDataFromID("area", "iAreaID", $txtid);
	if(empty($dataArr))
	{
		header("location: $disp_url");
		exit;
...
else if($mode=='U')
{
	...
	$values = " vName='$txtname', cStatus='$rdstatus', iPropertyID = '$cmbprop' ";
	$QUERY = UpdataData('area', $values, "iAreaID=$txtid");
	$_SESSION[PROJ_SESSION_ID]->success_info = "Area Details Successfully Updated";
}
elseif($mode=='D')
{
	$chk_arr = array();
	...
	if(!$chk)
	{
		DeleteData('area', 'iAreaID', $txtid);
		$_SESSION[PROJ_SESSION_ID]->success_info = "Area Deleted Successfully";
		$loc_str = $disp_url;
	}
	else
	{
		$_SESSION[PROJ_SESSION_ID]->alert_info = "Area Could Not Be Deleted Because of Existing ".(CHK_ARR2Str($chk_arr))." Dependencies";
	}
                ...
                <div class="col-md-12">
                  <div class="form-row">
                    <div class="col-md-12">
                      ...
                      <div class="position-relative form-group">
                        <label for="cmbloc" class="">Property <span class="text-danger">*</span></label>
                      	<?php echo FillCombo($cmbprop, 'cmbprop', 'COMBO', '0', $PROPERTY_ARR, '', 'form-control form-control-sm'); ?>
                      </div>					  
                    </div>
                  </div>
                  ...
                  <?php 
										...
										{
									?>
                  	<button type="button" onClick="SubmitIncludeForm('<?php echo $edit_url; ?>','D','<?php echo $txtid; ?>','Area');" class="mt-2 btn btn-danger">Delete</button>
                  <?php
										}
									?>
                </div>
...
$( document ).ready(function() {
	$("#dsgFrom").submit( function() {
		...
		if($.trim(cmbprop.val()) == 0 || $.trim(cmbprop.val()) == '') {
			ShowError( cmbprop, "Please select property");
			err_arr[err] = cmbprop;
			err ++;
		}
		else
