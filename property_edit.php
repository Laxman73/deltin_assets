...
$PAGE_TITLE2 = 'Property';

$PAGE_TITLE .= $PAGE_TITLE2;

$disp_url = 'property_disp.php';
$edit_url = 'property_edit.php';
...
$LOCATION_ARR = GetXArrFromYID("select iLocID, vName from location where cStatus = 'A'", 3);
$USER_REF_ID = array();
...
{
	$txtid = NextID('iPropertyID', 'property');
	...
	$q = "INSERT INTO property(iPropertyID, vName, vLogo, iLocID, cStatus, cShortCode) VALUES ($txtid, '$txtname', '$txtpic', '$cmbloc', '$rdstatus', '$txtshortcode')";
	$r = sql_query($q, "COMPANY.70");
	
	$_SESSION[PROJ_SESSION_ID]->success_info = "Property Details Successfully Inserted";
}
else if($mode=='E')
{
	$dataArr = GetDataFromID("property", "iPropertyID", $txtid);
	if(empty($dataArr))
	{
		header("location: $disp_url");
		exit;
	...
	$cmbloc = $dataArr[0]->iLocID;
	$txtpic = $dataArr[0]->vLogo;
	$txtshortcode = $dataArr[0]->cShortCode;
...
else if($mode=='U')
{
	...
	$values = " vName='$txtname', cStatus='$rdstatus', iLocID = '$cmbloc', cShortCode = '$txtshortcode' ";
	$QUERY = UpdataData('property', $values, "iPropertyID=$txtid");
	$_SESSION[PROJ_SESSION_ID]->success_info = "Property Details Successfully Updated";
}
elseif($mode=='D')
{
	$chk_arr = array();
	...
	$chk_arr['Area'] = GetCounts('area', " and iPropertyID=$txtid");

	$chk = array_sum($chk_arr);

	if(!$chk)
	{
		DeleteData('property', 'iPropertyID', $txtid);
		$_SESSION[PROJ_SESSION_ID]->success_info = "Property Deleted Successfully";
		$loc_str = $disp_url;
	}
	else
	{
		$_SESSION[PROJ_SESSION_ID]->alert_info = "Property Could Not Be Deleted Because of Existing ".(CHK_ARR2Str($chk_arr))." Dependencies";
	}
        ...
        <div class="row col-md-6">
          <div class="main-card mb-3 card col-md-12">
            <div class="card-header-tab card-header">
              <div class="card-header-title font-size-lg text-capitalize font-weight-normal"> <i class="header-icon pe-7s-culture mr-3 text-muted opacity-6"> </i><?php echo $modalTITLE; ?> </div>
            </div>
            <div class="card-body">
                ...
                <div class="col-md-12">
                  <div class="form-row">
                    <div class="col-md-12">
                      ...
                      <div class="position-relative form-group">
                        <label for="cmbloc" class="">Location <span class="text-danger">*</span></label>
                      	<?php echo FillCombo($cmbloc, 'cmbloc', 'COMBO', '0', $LOCATION_ARR, '', 'form-control form-control-sm'); ?>
                      </div>					  
                    </div>
                  </div>
                  ...
                  <div class="form-row">
                    <div class="col-md-6">
						<div class="position-relative form-group">
							<label for="examplePassword11" class="">Short Code</label>
                  ...
                  <?php 
										...
										{
									?>
                  	<button type="button" onClick="SubmitIncludeForm('<?php echo $edit_url; ?>','D','<?php echo $txtid; ?>','Property');" class="mt-2 btn btn-danger">Delete</button>
                  <?php
										}
									?>
                </div>
...
$( document ).ready(function() {
	$("#dsgFrom").submit( function() {
		...
		if($.trim(cmbloc.val()) == 0 || $.trim(cmbloc.val()) == '') {
			ShowError( cmbloc, "Please select location");
			err_arr[err] = cmbloc;
			err ++;
		}
		else
