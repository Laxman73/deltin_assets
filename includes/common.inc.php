...
if(sql_num_rows($r)){
    ...
    $row = sql_fetch_object($r);
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, 
                      ...
                      array('from' => 'Work Better Club <no-reply@workbetterclub.com>',
...
function GetBookmarkCount($member_id, $type='SES'){ //#6
...
function GetInProgressSelfPaced($member_id){ //#5
...
function GetCompletedSelfPaced($member_id, $series=false, $topic=false){ //#4
...
function GetSelfPacedTimeSpent($member_id){ //#7
...
function GetIncompleteSessions($member_id){ //#9
...
function GetSessionFeedback($member_id){ //#10
...
function GetSessionScores($member_id){ //#11
...
function GetMemberCertificates($member_id=''){ //#12, #32
...
function GetRatingAvg($series=false){ //#30, #31
...
function GetMemberQuestions($member_id){ //#18
...
function GetMemberNotes($member_id){ //#18
...
function AssessmentVSCertificates(){ //#33
...
function GetPopularity($type){ //#35, #36
...
function GetCertificatesFor($topics, $series){ //#34
...
function GetCompletedAttendedLive($member_id, $series=false, $topic=false){ //#3
...
function GetMemberTimeSpent($member_id){ //#13
...
function ManagerDashData($days='90'){

	$response_arr = $loc_arr = $property_arr = $dept_arr = array();

	$threshold_date = date('Y-m-d', strtotime("-".$months." days"));

	$response_str = '';

	$q = "select * from location where cStatus = 'A'";

	$r = sql_query($q, "common.inc.3794");

	$q2 = "select * from department where cStatus = 'A'";

	$r2 = sql_query($q2, "common.inc.3794");
	...
	if(sql_num_rows($r)){
		...
		while($row = sql_fetch_object($r)){
			...
			$loc_report_count = GetXFromYID("select count(*) from report where iLocID = '$row->iLocID' and dtCreation >= '$threshold_date 00:00:00'");

			$q1 = "select * from property where iLocID = '$row->iLocID'";
			$r1 = sql_query($q1, "common.inc.7604");
			$prop_arr = array();
			if(sql_num_rows($r1)){

				while($row1 = sql_fetch_object($r1)){
					$prop_report_count = GetXFromYID("select count(*) from report where iPropertyID = '$row1->iPropertyID' and dtCreation >= '$threshold_date 00:00:00'");
					$nodes = GetPropertyDeptData($row1->iPropertyID);
					$prop_arr[] = array("text"=>$row1->vName, "href"=>"report_disp.php?srch_mode=QUERY&status=&stage=&property=".$row1->iPropertyID, "tags"=>array($prop_report_count), "nodes"=>$nodes);
			...
			$response_arr[] = array("text"=>$row->vName, "href"=>"#", "tags"=>array($loc_report_count), "nodes"=>$prop_arr);
...
function GetPropertyDeptData($property){
	$resp_arr = array();
	$q = "select iDeptID, vName from department where cStatus = 'A'";
	$r = sql_query($q, "common.inc.7631");

	if(sql_num_rows($r)){

		while($row = sql_fetch_object($r)){

			$dept_report_count = GetXFromYID("select count(*) from report r join report_dept rd on r.iReportID = rd.iReportID where r.iPropertyID = '$property' and rd.iDepartmentID = '$row->iDeptID' and r.dtCreation >= '$threshold_date 00:00:00'");

			$resp_arr[] = array("text"=>$row->vName, "href"=>"report_disp.php?srch_mode=QUERY&property=".$property."&status=&stage=&dept=".$row->iDeptID, "tags"=>array($dept_report_count));
...
function send_mailgun($subject, $email, $contents, $attachment, $cc='', $site_title='Deltin Surveillance', $bcc='')

{
	...
	if(!empty($cc))
  	...
  	$message['bcc'] = 'david@teaminertia.com, '.$bcc; // 'joey@teaminertia.com, vernon@teaminertia.com';
  	//$message['bcc'] = 'david@teaminertia.com'; // 'joey@teaminertia.com, vernon@teaminertia.com';
	...
	if(is_array($attachment)){
		...
		for($a=0;$a<sizeof($attachment);$a++){
			$mi = $a+1;
			$message['attachment['.$mi.']'] = curl_file_create($attachment[$a], 'application/pdf', $attachment[$a]);

		}

	} else {

	$message['attachment'] = curl_file_create($attachment, 'application/pdf', $attachment);
...
function GenerateReportCode($property){
	
	$short_code = GetXFromYID("select cShortCode from property where iPropertyID = '$property'");
	
	$max_number = GetXFromYID("select MAX(iReportNum)+1 from report where iPropertyID = '$property'");
	
	$number = str_pad($max_number,4,"0",STR_PAD_LEFT);
	
	$code = $short_code."".$number;
