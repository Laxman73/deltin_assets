...
if(isset($_POST['srch_mode']) && $_POST['srch_mode']=='SUBMIT')
{
  ...
  $cmblocation = $_POST['cmblocation'];

	$params = '&keyword='.$txtkeyword.'&status='.$cmbstatus.'&location='.$cmblocation;
...
else if(isset($_GET['srch_mode']) && $_GET['srch_mode']=='QUERY')
{
  ...
  if(isset($_GET['location'])) $cmblocation = $_GET['location'];
...
if(!empty($cmblocation))
{
  $cond .= " and iLocID='".$cmblocation."'";
  $execute_query = true;
}
...
$dataArr = GetDataFromCOND("property", $cond.' order by vName');
        ...
        <div class="app-inner-layout">
              ...
              <div class="app-page-title">
                <div class="page-title-wrapper">
                  <form class="form-inline" name="frmSearch" id="frmSearch" action="<?php echo $disp_url; ?>" method="post">
                    ...
                    <div class="wm-100 mrm-50 position-relative form-group">
                      <?php echo FillCombo($cmblocation, 'cmblocation', 'COMBO', '0', $LOCATION_ARR, '', 'form-control form-control-sm'); ?>
                    </div>
          ...
          <div class="card mb-3">
            <div class="card-header-tab card-header">
              <div class="card-header-title font-size-lg text-capitalize font-weight-normal"> <i class="header-icon pe-7s-culture mr-3 text-muted opacity-6"> </i><?php echo $PAGE_TITLE2;  ?> </div>
                      ...
                      <tbody>
                        <?php 
                  				if(!empty($dataArr))
                  				{
                  					...
                  					{ 
                  						$i=$d+1;
                  						$x_id =  $dataArr[$d]->iPropertyID;
                  						$x_name = !empty($dataArr[$d]->vName)?db_output($dataArr[$d]->vName):'- Na -';
                  						$x_loc = !empty($dataArr[$d]->iLocID)?$LOCATION_ARR[$dataArr[$d]->iLocID]:'- Na -';
                  						$stat = $dataArr[$d]->cStatus;

                              $status_str = GetStatusImageString('PROPERTY', $stat, $x_id, true);
